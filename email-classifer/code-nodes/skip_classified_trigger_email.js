...
if (!messageId) {
  return [{
    json: {
      ...item,
      trigger_already_classified: false,
      trigger_classified_check: 'skipped_missing_message_id',
    },
  }];
}
...
if (dryRun) {
  return [{
    json: {
      ...item,
      trigger_already_classified: false,
      trigger_classified_check: 'dry_run_not_checked',
      trigger_classified_mailbox: stateMailbox,
    },
  }];
}
...
try {
  ...
  if (!(await client.mailboxExists(stateMailbox))) {
    return [{
      json: {
        ...item,
        trigger_already_classified: false,
        trigger_classified_check: 'state_mailbox_missing',
        trigger_classified_mailbox: stateMailbox,
      },
    }];
  }
...
return [{
  json: {
    ...item,
    trigger_already_classified: false,
    trigger_classified_check: 'not_classified',
    trigger_classified_mailbox: stateMailbox,
  },
}];
